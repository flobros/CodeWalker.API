...
public class ImportController : ControllerBase
{
        ...
        string? outputFolder)
    {
        ...
        try
        {
            ...
            if (rpfFile != null)
            {
                var isGen9 = rpfFile.Encryption == RpfEncryption.NG;

                _logger.LogInformation("[Import] Importing to archive: {Path}", rpfFile.FilePath ?? "(unknown)");
                _logger.LogInformation("[Import] Gen9 Archive: {IsGen9} | Encryption: {Encryption}", isGen9, rpfFile.Encryption);
            }
